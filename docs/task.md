...
### 5. 検索ロジックの実装
- [ ] チャットエージェント別検索ロジックの実装
  - チャットエージェントが紐づくドキュメントのみを検索対象とする
  - APIキーによるエージェント識別
- [ ] ドキュメント検索アルゴリズムの選択
...
### 8. APIエンドポイントの実装
- [ ] `/chat`エンドポイントの完全実装
  - APIキーによるチャットエージェント識別
  - エージェントが紐づくドキュメントのみを検索対象とする
...
- [ ] 認証・認可システムの実装
  ...
  - 未ログイン時のリダイレクト
- [ ] ルーティング設定
  - `/admin/documents` (index, new, create, edit, update, archive)
  - `/admin/chats` (index, show, new, create, edit, update, archive)
  - `/admin/chat_sessions` (index)
  - `/admin/issues` (index, update, bulk-update)
- [ ] ドキュメント管理画面
  - 一覧表示（ransack検索、kaminariページング）
  - 新規追加・編集・更新・アーカイブ機能
- [ ] チャット管理画面
  - チャットエージェント一覧・詳細・作成・編集
  - APIキーとシークレットの発行機能
  - ドキュメントとの1対1関連付け
  - エージェント別の回答範囲制御
- [ ] チャットセッション管理画面
  - セッション一覧（ransack検索、kaminariページング）
  - セッション詳細（chat_messages表示）
- [ ] イシュー管理画面
  - イシュー一覧（ransack検索、kaminariページング）
  - イシュー詳細（関連チャットセッション・ログ表示）
  - 一括更新機能
